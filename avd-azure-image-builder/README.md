Guillaume's comment (update file later) <

- The first time the script is being run, it fails with "ERROR: Deployment failed. Not authorized to access the resource: XYZ. Please check the user assigned identity has the correct permissions." This is a normal behavior due to the fact that a user assigned identity is created during the first run. Simply running the script again fixes the issue.
